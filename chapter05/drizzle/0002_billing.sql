...
CREATE TABLE `prices` (
    ...
    `input_price_per_token` integer NOT NULL, 
    `output_price_per_token` integer NOT NULL, 
    `model_multiplier` integer NOT NULL 1000, 
...
CREATE TABLE `usage_records` (
    ...
    `multiplier_snapshot` integer NOT NULL DEFAULT 0, 
...
CREATE UNIQUE INDEX `usage_records_trace_id_idx` ON `usage_records` (`trace_id`); 
CREATE INDEX `usage_records_user_time_idx` ON `usage_records` (`user_id`, `created_at`); 
CREATE INDEX `usage_records_key_time_ids` ON `usage_records` (`key_id`, `created_at`); 

// Chapter 6 v0.6 workspace — baseline is v0.5 billing; hand-type the rate-limit delta.
//
// Carried from v0.5:
//   1. prices / usage_records + users.balance_micro / user_multiplier
//   2. /v1/chat/completions: preConsume → upstream → postConsume / refundReservation
//   3. seedDefaultPricesIfEmpty() on startup
//   4. tiktoken estimate ↔ upstream usage dual ledger
//
// Hand-type in this chapter (empty stubs already created):
//   - src/limit/*              RateLimiter + QPS sliding window + TPM reserve/commit + middleware
//   - src/billing/quota.ts     monthly quota check + commit
//   - drizzle/0003_quota.sql   keys table: qps/tpm/monthly columns
//   - src/scripts/attack.ts    concurrent load script
//   - wire into schema.ts / admin/routes.ts / this file (auth → rateLimit → preConsume)
//
// Still later:
//   - Streaming (Ch7); channel pool (Ch8); observability (Ch9)
...
app.post('/v1/chat/completions', requireGatewayKey, async (c) => {
  ...
  if (ir.stream) {
    return c.json(
      {
        error: {
          message: 'streaming is not supported in v0.6; will be added in Ch7',
        },
      },
      400,
    );
    ...
    return c.json(
      {
        error: {
          message:
            '/v1/messages streaming passthrough is not implemented in v0.6; billing+stream in Ch7',
        },
      },
      400,
    );
